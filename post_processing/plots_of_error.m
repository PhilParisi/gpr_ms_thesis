...
figure()
for i = 1:6
    plot(average.dsample,average.RMSE), hold on
    plot(systematic.dsample,systematic.RMSE)
    plot(hybrid.dsample,hybrid.RMSE)
    plot(neighbor.dsample,neighbor.RMSE)
    plot(random.dsample,random.RMSE)
    plot(kmeans.dsample,kmeans.RMSE)
end

title('Accuracy vs. Downsample')
xlabel('Percent of Points Kept')
ylabel('Root Means Square Error (m)')
grid on
legend('average','systematic','hybrid','neighbor','random','kmeans')
