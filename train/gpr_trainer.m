...
for i = 1:length(L)
    ...
    hp.L = L(i);                   % lengthscale (high = smoother, low = noisier)
    hp.sigma_p = sigma_p(j);            % process noise (aka vertical scale, output scale)
    ...
    Y_Star_Hat = K_Star * inv(V) * Y;                  % Mean Predictions (mean of Gaussians)
...
%%
[L_mesh, sigma_mesh] = meshgrid(L(1:i),sigma_p(1:j));
plot3(L_mesh,sigma_mesh,maps_vals)
